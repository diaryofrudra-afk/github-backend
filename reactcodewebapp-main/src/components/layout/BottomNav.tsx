...
export function BottomNav({ onSignOut }: BottomNavProps) {
  ...
  // Notification count (pending items)
  const myTs: unknown[] = user ? ((state?.timesheets || {})[user] || []) : [];
  const notifCount = myTs.length > 0 ? Math.min(myTs.length, 99) : 0;

  const tabs = [
    {
      page: 'logger' as const, label: 'Log Time', icon: (
        <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" fill="none">
          ...
          >
            <span className="bottom-nav-icon">
              <div style={{
                ...
                {initials}
                {notifCount > 0 && (
                  <span style={{ position: 'absolute', top: -4, right: -4, background: '#ff3b30', color: '#fff', borderRadius: '50%', width: 14, height: 14, fontSize: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #fff' }}>
                    {notifCount}
                  </span>
                )}
              </div>
