...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    const authEndpoints = ['/auth/login', '/auth/verify-login-otp', '/auth/register', '/auth/test-login', '/sms-otp/send', '/sms-otp/verify'];
    const isAuthEndpoint = authEndpoints.some(ep => path.includes(ep));
    if (!isAuthEndpoint) {
      // Session is invalid/expired. Clear the FULL auth state — not just the
      // token — so the app re-renders the login screen instead of looking
      // logged-in (App.tsx gates on `rudra_user`). Clearing only the token
      // left those flags behind, producing a "looks logged in but every call
    ...
    try {
      ...
      if (Array.isArray(detail)) {
        throw new Error(
          detail
            .map((d: any) => {
              const loc = Array.isArray(d.loc) ? d.loc.filter((s: any) => s !== 'body') : [];
              const field = loc.join('.') || 'request';
              return `${field}: ${d.msg}`;
            })
            .join('; ')
        );
...
export const api = {
  ...
  testLogin(phone?: string): Promise<AuthResponse> {
    const qs = phone ? `?phone=${encodeURIComponent(phone)}` : '';
    return request('POST', `/auth/test-login${qs}`);
  },
  ...
  // Notifications
  getNotifications(userKey: string): Promise<Notification[]> {
    return request('GET', `/notifications?user_key=${encodeURIComponent(userKey)}`);
  },
  ...
  clearNotifications(userKey: string): Promise<void> {
    return request('DELETE', `/notifications/clear?user_key=${encodeURIComponent(userKey)}`);
  },
  ...
  // ── GPS Integrations ───────────────────────────────────────────────────────
  syncGPSToFleet(): Promise<{ added: number; updated: number }> {
    return request('POST', '/gps/sync-to-fleet');
  },

  blackbuckRequestOtp(phone: string): Promise<{ session_token: string }> {
    return request('POST', '/gps/blackbuck/request-otp', { phone });
  },

  blackbuckVerifyOtp(sessionToken: string, otp: string): Promise<void> {
    return request('POST', '/gps/blackbuck/verify-otp', { session_token: sessionToken, otp });
  },

  blackbuckAutoLogin(username: string, password: string): Promise<void> {
    return request('POST', '/gps/blackbuck/auto-login', { username, password });
  },

  trakntellAutoLogin(username: string, password: string): Promise<void> {
    return request('POST', '/gps/trakntell/auto-login', { username, password });
  },

  wheelseyeRequestOtp(phone: string): Promise<{ session_token: string }> {
    return request('POST', '/gps/wheelseye/request-otp', { phone });
  },

  wheelseyeVerifyOtp(sessionToken: string, otp: string): Promise<void> {
    return request('POST', '/gps/wheelseye/verify-otp', { session_token: sessionToken, otp });
  },
};
