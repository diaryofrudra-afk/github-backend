...
import type { BlackbuckVehicle, TrakNTellVehicle, WheelsEyeVehicle } from '../types';
...
export interface UnifiedVehicle {
  ...
  provider: 'blackbuck' | 'trakntell' | 'wheelseye';
}
...
export function useUnifiedGPS(opts?: { pollInterval?: number }) {
  ...
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      ...
      const [blackbuckResp, trakntellResp, wheelseyeResp] = await Promise.allSettled([
        ...
        fetch('/api/gps/wheelseye/vehicles', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
      ...
      if (wheelseyeResp.status === 'fulfilled' && wheelseyeResp.value.ok) {
        ...
        console.log('[GPS] WheelsEye response:', {
          hasVehicles: !!wheelseyeData.vehicles,
          vehicleCount: wheelseyeData.vehicles?.length,
          error: wheelseyeData.error,
        });
        ...
        console.log('[GPS] WheelsEye failed:', wheelseyeResp.status === 'rejected' ? wheelseyeResp.reason : `HTTP ${wheelseyeResp.value.status}`);
      }
