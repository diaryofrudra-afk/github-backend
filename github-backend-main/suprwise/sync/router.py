...
async def import_data(
    ...
    # operatorProfiles - Update operators table and operator profile docs
    for op_key, prof in (body.operatorProfiles or {}).items():
        cursor = await db.execute(
            "SELECT id FROM operators WHERE (phone = ? OR id = ?) AND tenant_id = ?",
            (op_key, op_key, tid),
        )
        ...
        profile_fields = {
            ...
            "aadhaar_doc": prof.get("aadhaar_doc", ""),
            "license_doc": prof.get("license_doc", ""),
            "bank": prof.get("bank", ""),
            "ifsc": prof.get("ifsc", ""),
        ...
        cursor = await db.execute(
            "SELECT id FROM operator_profiles WHERE operator_id = ? AND tenant_id = ?",
            (operator_id, tid),
        )
        ...
        if profile_row:
            await db.execute(
                """UPDATE operator_profiles SET
                   ...
                   account = ?,
                   address = ?
                   WHERE operator_id = ? AND tenant_id = ?""",
                (
                    profile_fields["photo"],
                    profile_fields["aadhaar_doc"],
                    profile_fields["license_doc"],
                    profile_fields["bank"],
                    profile_fields["ifsc"],
            ...
            await db.execute(
                """INSERT INTO operator_profiles
                   (id, operator_id, tenant_id, photo, aadhaar_doc, license_doc, bank, ifsc, account, address)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    str(uuid.uuid4()),
                    operator_id,
...
async def export_data(user=Depends(get_current_user), db=Depends(get_db)):
    ...
    # operatorProfiles
    cursor = await db.execute(
        "SELECT * FROM operator_profiles WHERE tenant_id = ?", (tid,)
    )
    operator_profile_rows = await cursor.fetchall()
    operator_profile_by_operator_id = {
        r["operator_id"]: dict(r)
        for r in operator_profile_rows
    }
    operator_profiles: dict = {}
    for r in operators:
        op_key = r.get("phone") or r.get("id")
        if op_key:
            profile = operator_profile_by_operator_id.get(r.get("id"), {})
            operator_profiles[op_key] = {
                "salary": float(r.get("salary", 0) or 0),
                "workingDays": float(r.get("working_days", 26) or 26),
                "photo": profile.get("photo", ""),
                "aadhaar_doc": profile.get("aadhaar_doc", ""),
                "license_doc": profile.get("license_doc", ""),
    ...
    # diagnostics → keyed by crane_reg
    diagnostics_rows = await fetch_all("diagnostics")
    diagnostics: dict = {}
    for r in diagnostics_rows:
        key = r.get("crane_reg", "")
        # Parse snapshot JSON if it exists
