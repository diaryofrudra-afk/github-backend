...
from .pdf_service import generate_billing_pdf
...
def _encode_json(value, default):
    """JSON-encode a list/dict for storage; pass-through strings."""
    if value is None:
        return default
    ...
    await db.execute(
        """INSERT INTO invoices
           (id, number, date, due_date, client_id, asset_reg, items,
            subtotal, sgst, cgst, total, status, paid_amount, notes,
            terms, signature_url, discount, additional_charges, total_in_words,
            custom_fields, advanced_options, shipping, currency, tenant_id)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            ...
            body.status, body.paid_amount, body.notes,
            _encode_json(body.terms, "[]"),
            body.signature_url,
            body.discount,
            body.additional_charges,
            body.total_in_words,
            _encode_json(body.custom_fields, "{}"),
            _encode_json(body.advanced_options, "{}"),
            _encode_json(body.shipping, "{}"),
            body.currency,
            user["tenant_id"],
        ),
    )
...
async def get_invoice_pdf(
    ...
    o_cursor = await db.execute(
        "SELECT * FROM owner_profiles WHERE tenant_id = ?",
        (user["tenant_id"],)
    )
    o_row = await o_cursor.fetchone()
    owner = dict(o_row) if o_row else None
    
    # Generate PDF
    try:
        pdf_data = await generate_billing_pdf("Invoice", invoice, client, owner)
    except Exception as e:
        logger.error(f"Failed to generate invoice PDF: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to generate PDF: {str(e)}")
        
    filename = f"invoice-{invoice.get('number', invoice_id)}.pdf"
    return Response(
        content=pdf_data,
...
async def get_quotation_pdf(
    ...
    # Generate PDF
    try:
        pdf_data = await generate_billing_pdf("Quotation", quotation, client, owner)
    except Exception as e:
        logger.error(f"Failed to generate quotation PDF: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to generate PDF: {str(e)}")
        
    filename = f"quotation-{quotation.get('number', quotation_id)}.pdf"
    return Response(
        content=pdf_data,
        media_type="application/pdf",
        headers={
