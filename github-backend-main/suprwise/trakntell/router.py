...
async def get_trakntell_trip_history(
    vehicle_id: str,
    from_date: str,   # "14/04/2026 00:00"
    to_date: str,     # "14/04/2026 23:59"
    user=Depends(get_current_user),
):
    """
    Fetch historical trip GPS points for a single Trak N Tell vehicle.
    Returns normalised list of { lat, lng, speed, timestamp, address }.
    """
    return await fetch_trakntell_trip_history(user_id=user["user_id"], vehicle_id=vehicle_id, from_date=from_date, to_date=to_date)
