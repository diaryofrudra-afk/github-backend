...
async def test_login(phone: str = "9010719021", db=Depends(get_db)):
    """Developer bypass endpoint to immediately login as test users."""
    cursor = await db.execute(
        "SELECT id, phone, email, email_verified, role, tenant_id FROM users WHERE phone = ?",
        (phone,),
    )
    row = await cursor.fetchone()
    if not row:
        raise HTTPException(404, f"Test user {phone} not found in database. Please register this phone number first.")
