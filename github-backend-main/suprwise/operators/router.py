...
async def get_my_operator_profile(user=Depends(get_current_user), db=Depends(get_db)):
    ...
    row = await cursor.fetchone()
    if not row:
        return {"operator_id": op["id"], "photo": "", "aadhaar_doc": "", "license_doc": "", "bank": "", "ifsc": "", "account": "", "address": ""}
    return dict(row)
...
async def update_my_operator_profile(
    ...
    if profile:
        ...
        await db.execute(
            """INSERT INTO operator_profiles (id, operator_id, tenant_id, photo, aadhaar_doc, license_doc, bank, ifsc, account, address)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (profile_id, operator_id, user["tenant_id"],
             data.get("photo") or "",
             data.get("aadhaar_doc") or "",
             data.get("license_doc") or "",
             data.get("bank") or "",
...
async def get_operator_profile(operator_id: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    row = await cursor.fetchone()
    if not row:
        return {"operator_id": operator_id, "photo": "", "aadhaar_doc": "", "license_doc": "", "bank": "", "ifsc": "", "account": "", "address": ""}
    return dict(row)
