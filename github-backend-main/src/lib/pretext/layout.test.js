...
let clearCache;
let setLocale;
let countPreparedLines;
let walkPreparedLines;
...
function getSegmentGraphemes(text) {
...
describe('prepare invariants', () => {
    ...
    test('treats soft hyphens as discretionary break points', () => {
        ...
        const softBreakWidth = Math.max(prefixed.widths[0] + prefixed.widths[1] + prefixed.widths[2] + prefixed.discretionaryHyphenWidth, prefixed.widths[4]) + 0.1;
        ...
        const continuedSoftBreakWidth = prefixed.widths[0] +
            prefixed.widths[1] +
            prefixed.widths[2] +
...
describe('layout invariants', () => {
    ...
    test('layoutNextLine variable-width streaming stays contiguous and reconstructs normalized text', () => {
        const prepared = prepareWithSegments('foo trans\u00ADatlantic said "hello" to 世界 and waved. According to محمد الأحمد, alpha\u200Bbeta 🚀', FONT);
