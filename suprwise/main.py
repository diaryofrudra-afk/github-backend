...
# Surface app logger.info() output (e.g. WheelsEye OTP diagnostics) in the uvicorn console.
logging.basicConfig(level=logging.INFO)
logging.getLogger("suprwise").setLevel(logging.INFO)
from fastapi.middleware.cors import CORSMiddleware
from .config import settings
from .database import init_db, close_db
from .gps_poller import start_gps_poller, stop_gps_poller
...
from .wheelseye.router import router as wheelseye_router
...
async def lifespan(app: FastAPI):
    await init_db()
    await start_gps_poller()
    yield
    await stop_gps_poller()
    await close_db()
...
app.include_router(wheelseye_router)
