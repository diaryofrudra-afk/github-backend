...
class TrakNTellCredentialsInput(BaseModel):
    ...
    username: str = ""    # TnT login username — stored for auto-refresh
    password: str = ""    # TnT login password — stored for auto-refresh
...
async def trakntell_health_check(user=Depends(get_current_user), db=Depends(get_db)):
    ...
    if creds_status:
        if creds_status.get("decryption_failed"):
        ...
        try:
            data = await fetch_trakntell_vehicle_data(user_id=user["user_id"])
...
async def set_trakntell_credentials(
    ...
    success = await store_credentials(
        ...
        tnt_username=creds.username or "",
        tnt_password=creds.password or "",
    )

    if success:
        msg = "Trak N Tell credentials saved successfully"
        if creds.username:
            msg += " (username/password stored for automatic session refresh)"
    ...
    if creds_status:
        if creds_status.get("decryption_failed"):
            return {
                ...
                "updated_at": creds_status["updated_at"],
                "last_error": "Decryption failed (InvalidToken). Secret may have changed."
            }
        return creds_status
...
async def trakntell_auto_login(
    ...
    success = await store_credentials(
        ...
        tnt_s=tnt_s,
        tnt_username=req.username,
        tnt_password=req.password,
    )
...
class ImmobilizerRequest(BaseModel):
    vehicle_id: str
    status: str  # "on" | "off"


@router.post("/toggle-immobilizer")
async def toggle_immobilizer(
    req: ImmobilizerRequest,
    user=Depends(require_owner),
):
    """
    Control the remote engine lock (immobilizer).
    ...
    cmd = "lock" if req.status == "on" else "unlock"
    params = {
        "f": cmd,
        "u": creds["user_id"],
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(
                "https://mapsweb.trakmtell.com/tnt/servlet/tntServiceSendCommand",
                params=params,
                cookies=cookies,
                headers=headers,
            )
        ...
        return {"ok": True, "command": cmd, "response": resp.text[:100]}
    except Exception as e:
...
async def trakntell_debug_raw(user=Depends(require_owner)):
    ...
    try:
        ...
        # Highlight ain/din/j1939 fields that have non-null values per vehicle
        sensor_summary = []
        for v in vehicles:
            entry = {"vehicle": v.get("vehicle_no") or v.get("nick_name") or v.get("vehicleid", "?"), "sensors": {}}
            for k, val in v.items():
                if (k.startswith("ain") or k.startswith("din") or k.startswith("j1939_") or
                        k in ("engine_hour", "today_engine_hours", "today_kms", "odometer")):
                    if val is not None and str(val).strip() not in ("", "0", "0.0", "null", "None"):
                        entry["sensors"][k] = val
            sensor_summary.append(entry)
...
async def discover_tnt_endpoints(user=Depends(require_owner)):
    ...
    try:
        from .auto_login import discover_endpoints_via_browser
    except ImportError:
        raise HTTPException(status_code=501, detail="discover_endpoints_via_browser is not implemented in auto_login.py")
