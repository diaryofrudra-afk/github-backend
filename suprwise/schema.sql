...
CREATE TABLE IF NOT EXISTS cranes (
    ...
    emi REAL NOT NULL DEFAULT 0.0,
    fixed_expenses REAL NOT NULL DEFAULT 0.0,
    tenant_id TEXT NOT NULL REFERENCES tenants(id),
...
CREATE TABLE IF NOT EXISTS engine_status_history (
    id TEXT PRIMARY KEY,
    crane_reg TEXT NOT NULL,
    engine_on BOOLEAN NOT NULL,
    previous_status BOOLEAN,
    changed_at TEXT NOT NULL DEFAULT (datetime('now')),
    source TEXT NOT NULL DEFAULT 'gps', -- gps, manual, api
    location_lat REAL,
    location_lng REAL,
    speed REAL,
    address TEXT,
...
CREATE INDEX IF NOT EXISTS idx_engine_status_crane ON engine_status_history(crane_reg, tenant_id);
CREATE INDEX IF NOT EXISTS idx_engine_status_date ON engine_status_history(changed_at, tenant_id);
CREATE TABLE IF NOT EXISTS tenants (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    created_at TEXT NOT NULL DEFAULT (datetime('now'))
...
CREATE TABLE IF NOT EXISTS operator_profiles (
    ...
    tenant_id TEXT NOT NULL REFERENCES tenants(id),
    photo TEXT NOT NULL DEFAULT '',
    aadhaar_doc TEXT NOT NULL DEFAULT '',
    license_doc TEXT NOT NULL DEFAULT '',
    bank TEXT NOT NULL DEFAULT '',
    ifsc TEXT NOT NULL DEFAULT '',
    account TEXT NOT NULL DEFAULT '',
    address TEXT NOT NULL DEFAULT '',
...
-- Per-user WheelsEye credentials (encrypted auth token at rest)
-- Captured via headless OTP login; vehicles_api_url is the live endpoint to replay.
CREATE TABLE IF NOT EXISTS wheelseye_credentials (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL UNIQUE REFERENCES users(id),
    tenant_id TEXT NOT NULL REFERENCES tenants(id),
    auth_token_encrypted TEXT NOT NULL,
    fleet_id TEXT NOT NULL DEFAULT '',
    vehicles_api_url TEXT NOT NULL DEFAULT '',
    cookies_encrypted TEXT NOT NULL DEFAULT '',
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_we_creds_user ON wheelseye_credentials(user_id);
CREATE INDEX IF NOT EXISTS idx_we_creds_tenant ON wheelseye_credentials(tenant_id);
