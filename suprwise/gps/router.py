...
from ..engine_status.service import log_engine_status_change
from ..engine_status.models import EngineStatusLogRequest
...
async def sync_all_gps_to_fleet(
    ...
    if not bb_data.error:
        for v in bb_data.vehicles:
            ...
            # Log engine status change if engine_on is not None
            if v.engine_on is not None:
                await log_engine_status_change(
                    db,
                    EngineStatusLogRequest(
                        crane_reg=reg,
    ...
    if not tnt_data.error:
        for v in tnt_data.vehicles:
            ...
            # Log engine status change for TNT vehicles
