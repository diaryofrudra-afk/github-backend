...
async def blackbuck_request_otp(phone: str) -> dict:
    ...
    try:
        ...
        # Normalize phone number — Blackbuck expects 10 digits only (no +91 / country code)
        import re as _re
        phone_digits = _re.sub(r'[^\d]', '', phone)  # strip +, spaces, hyphens
        if phone_digits.startswith('91') and len(phone_digits) == 12:
            phone_digits = phone_digits[2:]   # +91XXXXXXXXXX → XXXXXXXXXX
        elif phone_digits.startswith('0') and len(phone_digits) == 11:
            phone_digits = phone_digits[1:]   # 0XXXXXXXXXX → XXXXXXXXXX
        logger.info(f"[Blackbuck OTP] Normalized phone: {phone_digits[:4]}****")
        ...
        # Iframe fallback
        if not filled:
            filled = await _try_fill_in_frames(page, phone_selectors, phone_digits, "Blackbuck OTP", "phone")

        # JS fallback: fill first visible input by position
        if not filled:
            filled = await _js_fill_input(page, 0, phone_digits, "Blackbuck OTP", "phone")
...
async def blackbuck_verify_otp(session_token: str, otp: str) -> dict:
    ...
    try:
        ...
        # ── Extract browser session cookies (needed for portal API calls) ──
        portal_cookies = ""
        try:
            cookies_raw = await page.context.cookies()
            portal_cookies = json.dumps(cookies_raw)
            logger.info(f"[Blackbuck OTP] Extracted {len(cookies_raw)} browser cookies for portal API")
        except Exception as e:
            logger.warning(f"[Blackbuck OTP] Could not extract cookies: {e}")
