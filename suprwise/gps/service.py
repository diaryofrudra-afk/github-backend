...
_vehicles_cache: Dict[str, tuple[float, BlackbuckData]] = {}
...
async def _get_user_credentials(user_id: str) -> Optional[dict]:
    ...
    try:
        ...
        cursor = await db.execute(
            "SELECT auth_token_encrypted, fleet_owner_id, portal_cookies_encrypted FROM blackbuck_credentials WHERE user_id = ?",
            (user_id,),
        )
        row = await cursor.fetchone()
        if row:
            token = decrypt_token(row[0])
            # Decrypt and parse portal cookies if present (for /portal/* endpoints)
            portal_cookies_dict: dict = {}
            if len(row) > 2 and row[2]:
                try:
                    cookies_json = decrypt_token(row[2])
                    cookies_list = json.loads(cookies_json)
                    portal_cookies_dict = {c["name"]: c["value"] for c in cookies_list if c.get("name")}
                except Exception:
                    pass
            creds = {
                "auth_token": token,
                "fleet_owner_id": row[1],
                "portal_cookies_dict": portal_cookies_dict,
            }
...
def _map_vehicle(raw: dict) -> BlackbuckVehicle:
    ...
    lng = float(raw.get("longitude", 0.0) or 0.0)
    has_valid_coords = lat != 0 or lng != 0
    is_gps_working = has_valid_coords and status not in ("signal_lost", "wire_disconnected")
...
async def fetch_blackbuck_telemetry(user_id: Optional[str] = None, bypass_cache: bool = False) -> BlackbuckData:
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if resp.status_code == 401:
                res = BlackbuckData(error="Blackbuck token expired. Please update it in GPS Settings.")
            elif resp.status_code != 200:
                res = BlackbuckData(error=f"Blackbuck API error: {resp.status_code}")
            else:
                data = resp.json()
                raw_list = data.get("list", data.get("data", []))
                
                if not raw_list:
                    res = BlackbuckData(error="No vehicles found in Blackbuck account.")
                else:
                    vehicles = [_map_vehicle(v) for v in raw_list if v.get("truck_no")]
                    res = BlackbuckData(vehicles=vehicles)

    except httpx.TimeoutException:
        res = BlackbuckData(error="Connection to Blackbuck timed out.")
    except Exception as e:
        res = BlackbuckData(error=f"GPS fetch failed: {str(e)}")
...
async def fetch_blackbuck_trip_history(user_id: str, truck_no: str, from_time: int, to_time: int) -> TripHistoryResponse:
    ...
    token = creds["auth_token"]
    cookie_dict = creds.get("portal_cookies_dict") or {}
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.get(url, headers=_build_headers(token), cookies=cookie_dict)
