...
def _map_return_entries(entries) -> list:
    if not isinstance(entries, list):
        return []
    result = []
    for e in entries:
        result.append({
            "return_type": e.get("rtntype", ""),
            "tax_period": e.get("ret_prd", ""),
            "date_of_filing": e.get("dof", ""),
            "status": e.get("status", ""),
        })
    return result
...
@router.get("/balance")
async def get_gst_balance(user=Depends(get_current_user)):
    api_key = settings.GST_VERIFICATION_API_KEY
    if not api_key:
        return {"success": False, "error": "GST verification not configured"}
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get(
                f"http://sheet.gstincheck.co.in/balance/{api_key}"
            )
        result = resp.json()
    except Exception as e:
        return {"success": False, "error": f"GST API unreachable: {str(e)}"}
    return {"success": True, "credits": result.get("credits", result.get("balance", result))}


@router.post("/filing-status")
async def get_filing_status(req: GSTVerifyRequest, user=Depends(get_current_user)):
    gstin = req.gstin.strip().upper()
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get(
                f"http://sheet.gstincheck.co.in/check-return/{api_key}/{gstin}"
            )
        result = resp.json()
    except Exception as e:
        return {"success": False, "error": f"GST API unreachable: {str(e)}"}
    ...
    if not result.get("flag"):
        return {
            "success": False,
            "error": result.get("message", "No filing data found for this GSTIN"),
        }
    ...
    return {
        ...
        "data": {
            "gstin": gstin,
            "filing_status": _map_return_entries(result.get("data", [])),
        },
    }
