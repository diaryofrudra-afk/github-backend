...
async def process_vehicle_engine_status(
    db: aiosqlite.Connection,
    tenant_id: str,
    reg: str,
    ...
    *,
    ...
    """
    Shared edge-detection entry point used by both the manual sync-to-fleet endpoint
    and the background GPS poller.

    Normalizes the registration, logs an engine status change only on a real ON<->OFF
    transition, and (optionally) creates an in-app notification for that transition.
    Returns the change event if the status flipped, else None.
    """
    ...
    change = await log_engine_status_change(
        db,
    ...
    if change and create_notification and user_key:
        # Imported here to avoid a circular import at module load time.
        from ..notifications.service import create_notification as _create_notification
        from ..notifications.service import engine_notification_text
        await _create_notification(
            db,
            tenant_id,
            user_key,
            engine_notification_text(reg, engine_on, address),
            type="success" if engine_on else "warn",
        )
