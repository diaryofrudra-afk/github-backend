...
async def wheelseye_verify_otp_endpoint(
    ...
    logger.info(f"[WheelsEye Router] Verifying OTP with session_token len={len(req.session_token)}")
    ...
    if not auth_token or not auth_token.strip():
        raise HTTPException(status_code=400, detail="WheelsEye API returned empty auth token. Login failed. Please try again.")

    fleet_id: str = login_result.get("fleet_id") or ""
    vehicles_api_url: str = login_result.get("vehicles_api_url") or ""
    cookies_raw: str = login_result.get("cookies_raw") or ""

    encrypted = encrypt_token(auth_token)
    encrypted_cookies = encrypt_token(cookies_raw) if cookies_raw else ""
    ...
    try:
        ...
        if existing:
            await db.execute(
                ...
                "vehicles_api_url = ?, cookies_encrypted = ?, updated_at = datetime('now') WHERE user_id = ?",
                (encrypted, fleet_id, vehicles_api_url, encrypted_cookies, user["user_id"]),
            )
        else:
            await db.execute(
                "INSERT INTO wheelseye_credentials (id, user_id, tenant_id, auth_token_encrypted, fleet_id, vehicles_api_url, cookies_encrypted) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (str(uuid.uuid4()), user["user_id"], user["tenant_id"], encrypted, fleet_id, vehicles_api_url, encrypted_cookies),
            )
        await db.commit()
    except Exception as e:
        logger.error(f"[WheelsEye Router] Failed to save credentials: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to save WheelsEye credentials: {e}")
