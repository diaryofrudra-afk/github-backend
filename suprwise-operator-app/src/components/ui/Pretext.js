...
function balancedWidth(words, font, naturalWidth) {
    if (words.length <= 1)
        return naturalWidth;
...
function wordWrapCount(words, font, maxWidth) {
    let lines = 1;
    let lineWidth = 0;
...
export function Pretext({ text, font, balanced = false, className }) {
    const ref = useRef(null);
    const [maxWidth, setMaxWidth] = useState(null);
    useEffect(() => {
        if (!balanced || !ref.current)
            return;
        const compute = () => {
    ...
    return (_jsx("span", { ref: ref, className: className, style: style, children: text }));
}
