...
export function LoggerPage({ active }: { active: boolean }) {
  ...
  // Robust lookup: try user phone, then operator ID, then operator phone
  const profile = (user ? state.operatorProfiles[user] : null) || 
                  (currentOp?.id ? state.operatorProfiles[currentOp.id] : null) ||
                  (currentOp?.phone ? state.operatorProfiles[currentOp.phone] : null);

  const opFirstName = profile?.firstName || profile?.first_name || (currentOp?.name ? currentOp.name.split(' ')[0] : null);
      ...
      <div className="logger-total-row">
        ...
        <div className="logger-total-actions">
          ...
          <button className="logger-save-btn-rect" onClick={handleCommit}>
            SAVE SHIFT
            <span className="material-symbols-outlined" style={{ marginLeft: '8px' }}>check_circle</span>
          </button>
        </div>
      </div>
              ...
              return (
                <div key={e.id} className="logger-entry-swipe-wrapper">
                  ...
                  <div className="logger-entry-bottom-row">
                    {hasLogbook ? (
                      <div className="logger-entry-logbook-status">
                        <div className="logger-pill-icon logger-pill-icon-doc">
                          <span className="material-symbols-outlined" style={{ fontSize: '20px', color: 'var(--accent)' }}>assignment</span>
                        </div>
                        <span className="logger-pill-text">Logbook Attached</span>
                        <button className="logger-pill-action-btn" onClick={() => {
                      ...
                      <div className="logger-entry-logbook-status">
                        ...
                        <label className="logger-pill-action-btn" htmlFor={`upload-${e.id}`}>
                          Upload
                          <input
                            id={`upload-${e.id}`}
                            type="file"
