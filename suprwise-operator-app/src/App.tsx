  ...
  function loadDataFromAPI() {
    ...
    api.exportAll()
      .then((data: AppState) => {
        ...
        const opProfilesRaw = (raw.operator_profiles || raw.operatorProfiles || {}) as Record<string, any>;
        const operatorProfiles: any = {};
        for (const [id, p] of Object.entries(opProfilesRaw)) {
          operatorProfiles[id] = {
            photo: p.photo,
        ...
        const ownerProfileRaw = (raw.owner_profile || raw.ownerProfile || {}) as any;
        const ownerProfile = {
          name: ownerProfileRaw.name || '',
          firstName: ownerProfileRaw.first_name || ownerProfileRaw.firstName || '',
          lastName: ownerProfileRaw.last_name || ownerProfileRaw.lastName || '',
          roleTitle: ownerProfileRaw.role_title || ownerProfileRaw.roleTitle || '',
          phone: ownerProfileRaw.phone || '',
          email: ownerProfileRaw.email || '',
          company: ownerProfileRaw.company || '',
          city: ownerProfileRaw.city || '',
          state: ownerProfileRaw.state || '',
          gst: ownerProfileRaw.gst || '',
          website: ownerProfileRaw.website || '',
          defaultLimit: ownerProfileRaw.default_limit || ownerProfileRaw.defaultLimit || '8',
          photo: ownerProfileRaw.photo || '',
        };
